...
public class SeasonModel
{
	...
	throws SQLException
	{
		...
		{
			conn=dataSource.getConnection();
			ps=conn.prepareStatement("SELECT t.member_id, m.name, t.event_id, MIN(time) AS best_time FROM gt4_time t, gt4_event e, member m WHERE m.id=t.member_id AND e.season_id=? AND t.event_id=e.id GROUP BY t.event_id, t.member_id ORDER BY t.event_id, best_time");
			ps.setInt(1,seasonId);
			rs=ps.executeQuery();
			while(rs.next())
			{
				...
				srd.setUsername(rs.getString("name"));
				results.add(srd);
			}
		}
		finally
